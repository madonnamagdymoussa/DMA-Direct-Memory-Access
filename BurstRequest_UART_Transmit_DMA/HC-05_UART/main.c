...
int main(void)
{
    ...
    /* GPIO_WriteBitBandBits(Channel_1, PortF, OutputLow);
     ...
     //*******************Code for Mem To peripheral DMA UART Transmit [==Burst Request==]*******************
      GPIO_WriteBitBandBits(Channel_1, PortF, OutputLow);
      GPIO_WriteBitBandBits(Channel_1, PortF, OutputHigh);
      ...
      UART_Transmit_DMA(UART_Channel1,  &SourceBufferUART[7], DMAChannel_23);
      DMA_EnableChannel(DMAChannel_23);
      DMA_ConfigureChannelSoftwareRequest(EnableSoftwareRequest , DMAChannel_23);

      //*******************Code for Mem To peripheral Polling UART Transmit *******************
        /*UART_Initialization(&UARTChannel1_Config, GPIO_AFSELUARTConfig_ArrPtrStruct[1]);
        u8_t indexCounter=0;
        ...
        for(indexCounter=0;indexCounter<8;indexCounter++){
            UART_TransmitChar_Polling(UART_Channel1, &SourceBufferUART[indexCounter]);
        }
