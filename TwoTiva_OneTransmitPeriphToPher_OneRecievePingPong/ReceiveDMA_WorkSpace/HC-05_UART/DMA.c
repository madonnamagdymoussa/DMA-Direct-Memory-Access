...
DMA_SoftwareTransferHandler(){
    ...
    if(1==DMA_InterruptFlag){
        ...
        u8_t var2=0;
        if(0x10==DestBufferB[0]){
            var2=0x2;
        }
        if(0x01==DestBufferB[0]){
            var2=0x1;
        }

        *((volatile unsigned long*)0x400250FC)=((var2)<<1);
    }
    else if(0 == DMA_InterruptFlag){
        ...
        u8_t var1=0;
        if(0x10==DestBufferA[0]){
            var1=0x2;
        }
        if(0x01==DestBufferA[0]){
            var1=0x1;
        }
        *((volatile unsigned long*)0x400250FC)=((var1)<<1);
    }
...
/*     GPIO_WriteBitBandBits(Channel_1, PortF, OutputHigh);
   ...
   // for(delayCounter=0;delayCounter<500;delayCounter++);
    ...
    (*DMA_ChannelControlWordRegisters[DMAChannel_23]).Bits.XFERSIZE=7;
   ...
   //UART_Transmit_DMA(UART_Channel1,  &SourceBuffer_DMA[SourceIndexCounterDMA], DMAChannel_23);
   (*DMA_ChannelControlWordRegisters[DMAChannel_22]).Bits.XFERSIZE=15;
