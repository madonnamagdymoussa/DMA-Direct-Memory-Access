...
/************************************ Includes ************************************/
  ...
  void UART_ConfigureInterruptSource(enum UART_IRQ_Event InterruptFlag, UART_InterruptMaskConfig_t InterruptMaskConfig, UART_Channel_t Channel){
   ...
   void UART_TransmitChar_Interrupt(UART_Channel_t channel, u8_t *pRxBuffer){
    ...
    void UART_Transmit_DMA(UART_Channel_t Channel, u32_t *SourceAddressBuffer, DMAChannelNum_t DMAChannelNum){

         //DMA_ChannelsConfigTX[Channel] --> array of pointers to structures
          DMA_ChannelControlStructureSet(DMA_ChannelsConfigTX[Channel], (u32_t*)SourceAddressBuffer ,(u32_t*)UART_DataRegisters[Channel]  );
         (*UART_DMAControlRegisters[Channel]).bits.TransmitDMAEnable=1;
         (*UART_ControlRegisters[Channel]).bits.TransmitEnable=1;
     }

     void UART_Receive_DMA(UART_Channel_t Channel, u32_t *destAddressBuffer, DMAChannelNum_t DMAChannelNum){

         DMA_ChannelControlStructureSet(DMA_ChannelsConfigRX[Channel],(u32_t*)UART_DataRegisters[Channel],(u32_t*)destAddressBuffer );
